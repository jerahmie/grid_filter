...
def gen_random_plots(kd2d: KDTree2D, pts: List[Tuple[float, float]], n: int)->None:
    ...
    with open("compared_points.txt", 'w') as fp:
        fp.write('i, test_pt, nearest_cell_id, pts[nearest_cell_id], n compares\n' )
        for i in range(n):
            print(f'point: {i}')
            ...
            fp.write('{i}, {test_pt}, {nearest_cell_id}, ' + \
                     '{pts[nearest_cell_id]}, {kd2d.compares}\n' )
            ...
            gf.overplot_mpas_grid(ax, np.array([test_pt[0]]),
                                  np.array([test_pt[1]]), color='cyan')
