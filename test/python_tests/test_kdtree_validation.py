...
class TestKDTreeValidation(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.ptsi = [(180.0/np.pi*cls.pts[i][0], 180.0/np.pi*cls.pts[i][1], i) \
                     for i in range(len(cls.pts))]
        cls.kd2d= gf.KDTree2D(cls.ptsi)
    ...
    def test_bdy_msk_types(self):
        '''Test full kdtree'''
        ptsi = [(180.0/np.pi*self.pts[i][0], 180.0/np.pi*self.pts[i][1], i) \
    ...
    def test_find_min_max(self):
        ...
        '''
        max_lat = max([pt[0] for pt in self.ptsi[:]])
        min_lat = min([pt[0] for pt in self.ptsi[:]])
        max_lon = max([pt[1] for pt in self.ptsi[:]])
        min_lon = min([pt[1] for pt in self.ptsi[:]])
        node_stack = []
    ...
    def test_find_tree_min_max(self):
        ...
        self.assertTrue(np.abs(gf.find_tree_min_max(self.kd2d.root)[0][0] \
                - min_lat) < PRECISION)
        self.assertTrue(np.abs(gf.find_tree_min_max(self.kd2d.root)[0][1] \
                - max_lat) < PRECISION)
        self.assertTrue(np.abs(gf.find_tree_min_max(self.kd2d.root)[1][0] \
                - min_lon) < PRECISION)
        self.assertTrue(np.abs(gf.find_tree_min_max(self.kd2d.root)[1][1] \
                - max_lon) < PRECISION)
