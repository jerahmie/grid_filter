...
import os
import unittest
from grid_filter import MPASGrid, len_non_zero, \
        border_cell_ids_from_cells_per_vertices, filter_bdy_mask_cell
...
class TestGridPerimeter(unittest.TestCase):
    ...
    def test_filter_bdy_mask_cell(self):
        ...
        self.assertTrue(any(filter(lambda x: True if (x==6 or x==7) else False, \
                fbdy_mask)))
